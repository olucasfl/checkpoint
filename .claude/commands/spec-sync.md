...
2. Monte três colunas:
   ...
   - **Testado** — qual teste garante isso hoje (`arquivo:linha`), ou "nenhum" (inclusive se o
     workspace ainda não tem runner configurado — diga isso).
3. Classifique cada critério:
   ...
   - **código órfão** — o código faz algo que a spec não pede mais. **Não apague por conta
     própria**: reporte e pergunte.
4. Verifique os efeitos que a spec sozinha não mostra:
   - A mudança exige alteração de `apps/api/prisma/schema.prisma`? É **aditiva ou destrutiva**?
     Precisa de `/db-change`?
   - Muda contrato de rota (path, shape, status) ou de um tipo em `packages/shared`? Então o outro
     lado do monorepo (`apps/api` ↔ `apps/web`) precisa mudar em lockstep.

## Saída

Uma tabela com as colunas (critério · desejado · implementado · testado · veredito), e depois **um
plano curto** do que precisa mudar, em ordem, marcando o que é pendência humana.
