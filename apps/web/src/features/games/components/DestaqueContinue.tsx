...
const CHIP =
  'chip-escuro inline-flex h-[26px] items-center gap-1.5 rounded-full px-2.5 text-xs font-bold text-texto md:h-8 md:px-3.5 md:text-sm';
...
export function DestaqueContinue({ game }: { game: Game }) {
  ...
  return (
    <section
      ...
      className={`relative h-44 overflow-hidden rounded-[20px] md:h-[230px] md:rounded-3xl ${imagem ? 'bg-painel-2' : coverBackground(game.titulo)}`}
    >
      {imagem ? (
        <img
          src={imagem}
        ...
        <span
          aria-hidden="true"
          className="absolute -bottom-7 -right-2 font-display text-[170px] font-black leading-none text-fundo/30 md:-bottom-10 md:right-12 md:text-[230px]"
        >
          {coverInitials(game.titulo)}
        </span>
      )}
      <div aria-hidden="true" className="destaque-scrim absolute inset-0" />

      <div className="relative flex h-full max-w-[60%] flex-col justify-center gap-2 p-5 md:max-w-[50%] md:gap-3 md:px-10 md:py-8">
        <div
          id={rotuloId}
        ...
        <h2 className="tile-titulo m-0 font-display text-[32px] font-extrabold leading-none tracking-[-0.02em] text-texto md:text-[52px]">
          {game.titulo}
        </h2>
        ...
        >
          <Icon name="visibility" size={20} className="hidden md:inline-block" />
          Ver detalhes
        </Link>
      </div>
    </section>
