...
const OLD = `${ID}/00000000-0000-4000-8000-000000000000.png`;
...
describe('GamesService — capa', () => {
  ...
  describe('setCover', () => {
    it('sobe <id>/<uuid>.png, grava o capaPath e devolve a capaUrl sem vazar o capaPath (CA-53, CA-60)', async () => {
      ...
      const result = await service.setCover(ID, { buffer: PNG });
      ...
      expect(game.update).toHaveBeenCalledWith({ where: { id: ID }, data: { capaPath: path } });
    ...
    it.each([
      ...
      async (_nome, buffer, extension, mime) => {
        ...
        await service.setCover(ID, { buffer });
    ...
    it('sem capa antiga, não chama remove', async () => {
      ...
      await service.setCover(ID, { buffer: PNG });
    ...
    it.each([
      ...
      async (_nome, buffer) => {
        ...
        const error = await failure(service.setCover(ID, { buffer }));
    ...
    it.each([
      ...
      async (_nome, file) => {
        ...
        const error = await failure(service.setCover(ID, file));
    ...
    it('jogo inexistente → 404, mesmo com arquivo inválido, e nada é enviado (CA-59)', async () => {
      const { service, game, storage } = setup();
      game.findUnique.mockResolvedValue(null);

      const error = await failure(service.setCover(ID, { buffer: Buffer.from('texto') }));
    ...
    it('falha do storage no upload → 502 e o capaPath fica como estava (CA-63)', async () => {
      ...
      const error = await failure(service.setCover(ID, { buffer: PNG }));
    ...
    it('falha do banco depois do upload apaga o objeto novo, para não deixá-lo órfão', async () => {
      ...
      await expect(service.setCover(ID, { buffer: PNG })).rejects.toThrow('conexao perdida');
  ...
  describe('removeCover', () => {
    it('apaga o objeto ANTES de zerar o capaPath (CA-64)', async () => {
      ...
      const result = await service.removeCover(ID);

      expect(storage.remove).toHaveBeenCalledWith(OLD);
      expect(game.update).toHaveBeenCalledWith({ where: { id: ID }, data: { capaPath: null } });
    ...
    it('jogo inexistente → 404 (CA-64)', async () => {
      const { service, game } = setup();
      game.findUnique.mockResolvedValue(null);

      const error = await failure(service.removeCover(ID));
  ...
  describe('capaUrl nas respostas (CA-60)', () => {
    it('list e update trazem a capaUrl e nunca o capaPath', async () => {
      ...
      const updated = await service.update(ID, { nota: 8 });
    ...
    it('jogo criado não tem capa', async () => {
      ...
      const created = await service.create({ titulo: 'Celeste', status: 'ZERADO' });
